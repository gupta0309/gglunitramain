...
const LeadershipIncomeRewards = () => {
  ...
  const { 
    data: apiResponse = { history: [], totalPages: 1, totalRecords: 0, totalLeadershipIncome: 0 }, 
    isFetching, 
    isError 
  } = useQuery({
    queryKey: ["leadershipIncomeRewards", pagination.pageIndex, userIdFilter, startDate, endDate],
    queryFn: async () => {
      const pageNum = pagination.pageIndex + 1;
      const res = await adminApi.getLeadershipIncomeReport(pageNum, 10, startDate, endDate, userIdFilter);
      console.log(`📡 Leadership Income Page ${pageNum} fetched:`, res?.data);

      return {
        history: res?.data?.history || [],
        totalPages: res?.data?.totalPages || 1,
        totalRecords: res?.data?.totalRecords || 0,
        totalLeadershipIncome: res?.data?.totalLeadershipIncome || 0,
      };
    },
  ...
  const leadershipRewards = apiResponse.history;
  const pageCount = apiResponse.totalPages;
  const totalRecords = apiResponse.totalRecords;
  const totalLeadershipIncome = apiResponse.totalLeadershipIncome;
  ...
  // Fetch ALL records for export (with current filters)
  const fetchFullData = async () => {
    let all = [], page = 1, totalPages = 1;
    while (page <= totalPages) {
      const res = await adminApi.getLeadershipIncomeReport(page, 500, startDate, endDate, userIdFilter);
      const data = res?.data || {};
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text("Leadership Income Rewards Report", 14, 15);

      const headers = ["Sr No.", "User ID", "Rank", "Shares", "Pool Amount", "Total Shares", "Bonus Amount", "Month", "Assigned At"];
      ...
      for (let i = 0; i < allData.length; i += chunkSize) {
        const rows = allData.slice(i, i + chunkSize).map((item, idx) => [
          ...
          `$${Number(item.poolAmount || 0).toFixed(2)}`,
          Number(item.totalShares || 0).toFixed(0),
          `$${Number(item.bonusAmount || 0).toFixed(2)}`,
          item.month || "N/A",
      ...
      doc.save("leadership_income_rewards.pdf");
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const excelData = allData.map((item, idx) => ({
        ...
        "Pool Amount": `$${Number(item.poolAmount || 0).toFixed(2)}`,
        "Total Shares": Number(item.totalShares || 0).toFixed(0),
        "Bonus Amount": `$${Number(item.bonusAmount || 0).toFixed(2)}`,
        "Month": item.month || "N/A",
        "Assigned At": item.assignedAt ? new Date(item.assignedAt).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" }) : "N/A",
      }));

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(excelData), "LeadershipIncomeRewards");
      XLSX.writeFile(wb, "leadership_income_rewards.xlsx");
  ...
  return (
    <div className="p-4 max-w-[1260px] mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-[#103944]">Leadership Income Rewards</h2>

      {/* Total Summary */}
      <div className="mb-6 rounded-2xl p-4 bg-gradient-to-br from-gray-100 to-gray-50 border border-gray-200 shadow">
        <div className="text-sm text-gray-600">Total Leadership Income</div>
        <div className="text-2xl font-semibold mt-1">${Number(totalLeadershipIncome).toFixed(2)}</div>
      </div>
      ...
      <div className="relative overflow-x-auto bg-white rounded-lg shadow border min-h-[400px]">
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {leadershipRewards.length === 0 && !isFetching ? (
              <tr><td colSpan={9} className="p-12 text-center text-gray-500">No records found</td></tr>
            ) : (
...
export default LeadershipIncomeRewards;