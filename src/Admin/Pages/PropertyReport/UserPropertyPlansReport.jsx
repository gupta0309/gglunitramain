...
const InvestmentReport = () => {
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      console.log(`📡 Investment Page ${pageNum} fetched:`, res?.data);
  ...
  const fetchFullData = async () => {
    ...
    while (page <= totalPages) {
      const res = await adminApi.getAllInvestments(page);
      const data = res?.data || {};
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const headers = ["S.No.", "User ID", "Property ID", "Investment Type", "Amount USD", "Units Bought", "Entry Price", "Rental / Stake", "Status", "Created At"];
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const excelData = allData.map((item, idx) => ({
        ...
        "Amount USD": `$${Number(item.amount_usd || 0).toFixed(2)}`,
