...
const LevelIncomeRewards = () => {
  ...
  const { 
    data: apiResponse = { history: [], totalPages: 1, totalRecords: 0, totalLevelIncome: 0 }, 
    isFetching, 
    isError 
  } = useQuery({
    queryKey: ["levelIncomeRewards", pagination.pageIndex, userIdFilter, startDate, endDate],
    queryFn: async () => {
      const pageNum = pagination.pageIndex + 1;
      const res = await adminApi.getLevelIncomeReport(pageNum, 10, startDate, endDate, userIdFilter);
      console.log(`📡 Level Income Page ${pageNum} fetched:`, res?.data);

      return {
        history: res?.data?.history || res?.data?.referrals || [],
        totalPages: res?.data?.totalPages || 1,
        totalRecords: res?.data?.totalRecords || 0,
        totalLevelIncome: res?.data?.totalLevelIncome || res?.data?.totalReferralIncome || 0,
      };
    },
  ...
  const levelRewards = apiResponse.history;
  const pageCount = apiResponse.totalPages;
  const totalRecords = apiResponse.totalRecords;
  const totalLevelIncome = apiResponse.totalLevelIncome;
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: "fromUserId", header: "From User ID" },
  ...
  const fetchFullData = async () => {
    ...
    while (page <= totalPages) {
      const res = await adminApi.getLevelIncomeReport(page, 500);
      const data = res?.data || {};
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const headers = ["Sr No.", "User ID", "From User ID", "Invested Amt", "Level", "Amount (in $)", "Distribution Date"];
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const excelData = allData.map((item, idx) => ({
        ...
        "From User ID": item.fromUserId || "N/A",
      ...
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(excelData), "LevelIncomeRewards");
      XLSX.writeFile(wb, "level_income_rewards.xlsx");
  ...
  return (
    <div className="p-4 max-w-[1260px] mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-[#103944]">Level Income Rewards</h2>

      {/* Filter Section - Same as Daily ROI */}
      ...
      {/* Export Buttons - Same as Daily ROI */}
      ...
      <div className="relative overflow-x-auto bg-white rounded-lg shadow border min-h-[400px]">
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {levelRewards.length === 0 && !isFetching ? (
      ...
      {/* Pagination - Same as Daily ROI (First | Prev | Next | Last) */}
