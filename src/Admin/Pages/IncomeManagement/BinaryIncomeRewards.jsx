...
const BinaryIncomeRewards = () => {
  // Filters (same as Daily ROI & Level Income)
  ...
  const { 
    data: apiResponse = { history: [], totalPages: 1, totalRecords: 0, totalBinaryIncome: 0 }, 
    isFetching, 
    isError 
  } = useQuery({
    queryKey: ["binaryIncomeRewards", pagination.pageIndex, userIdFilter, startDate, endDate],
    queryFn: async () => {
      const pageNum = pagination.pageIndex + 1;
      const res = await adminApi.getBinaryIncomeReport(pageNum, 10, startDate, endDate, userIdFilter);
      console.log(`📡 Binary Income Page ${pageNum} fetched:`, res?.data);

      return {
        history: res?.data?.history || [],
        totalPages: res?.data?.totalPages || 1,
        totalRecords: res?.data?.totalRecords || 0,
        totalBinaryIncome: res?.data?.totalBinaryIncome || 0,
      };
    },
  ...
  const binaryRewards = apiResponse.history;
  const pageCount = apiResponse.totalPages;
  const totalRecords = apiResponse.totalRecords;
  const totalBinaryIncome = apiResponse.totalBinaryIncome;
  ...
  const columns = useMemo(() => [
    ...
    { accessorKey: "leftChildInvestment", header: "Left Investment + Carry Forward" },
    { accessorKey: "rightChildInvestment", header: "Right Investment + Carry Forward" },
    { accessorKey: "leftCarryForward", header: "Left Carry Forward" },
    { accessorKey: "rightCarryForward", header: "Right Carry Forward" },
    { accessorKey: "previousLeftCarry", header: "Previous Left Carry" },
    { accessorKey: "previousRightCarry", header: "Previous Right Carry" },
  ...
  const fetchFullData = async () => {
    ...
    while (page <= totalPages) {
      const res = await adminApi.getBinaryIncomeReport(page, 500, startDate, endDate, userIdFilter);
      const data = res?.data || {};
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text("Binary Income Rewards Report", 14, 15);

      const headers = [
        "Sr No.", "User ID", "Left Investment + Carry", "Right Investment + Carry",
        "Left Carry Forward", "Right Carry Forward", "Previous Left Carry",
        "Previous Right Carry", "Matches", "Amount (in $)", "Distribution Date"
      ];
      ...
      for (let i = 0; i < allData.length; i += chunkSize) {
        const rows = allData.slice(i, i + chunkSize).map((item, idx) => [
          i + idx + 1,
          item.user_id || "N/A",
          Number(item.leftChildInvestment || 0).toFixed(2),
          Number(item.rightChildInvestment || 0).toFixed(2),
          Number(item.leftCarryForward || 0).toFixed(2),
          Number(item.rightCarryForward || 0).toFixed(2),
          Number(item.previousLeftCarry || 0).toFixed(2),
          Number(item.previousRightCarry || 0).toFixed(2),
          Number(item.matches || 0).toFixed(2),
      ...
      doc.save("binary_income_rewards.pdf");
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const excelData = allData.map((item, idx) => ({
        ...
        "Left Investment + Carry Forward": Number(item.leftChildInvestment || 0).toFixed(2),
        "Right Investment + Carry Forward": Number(item.rightChildInvestment || 0).toFixed(2),
        "Left Carry Forward": Number(item.leftCarryForward || 0).toFixed(2),
        "Right Carry Forward": Number(item.rightCarryForward || 0).toFixed(2),
        "Previous Left Carry": Number(item.previousLeftCarry || 0).toFixed(2),
        "Previous Right Carry": Number(item.previousRightCarry || 0).toFixed(2),
        "Matches": Number(item.matches || 0).toFixed(2),
        "Amount (in $)": `$${Number(item.amount || 0).toFixed(2)}`,
        "Distribution Date": item.distributionDate 
          ? new Date(item.distributionDate).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" }) 
          : "N/A",
      }));

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(excelData), "BinaryIncomeRewards");
      XLSX.writeFile(wb, "binary_income_rewards.xlsx");
  ...
  return (
    <div className="p-4 max-w-[1260px] mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-[#103944]">Binary Income Rewards</h2>

      {/* Total Summary */}
      <div className="mb-6 rounded-2xl p-4 bg-gradient-to-br from-gray-100 to-gray-50 border border-gray-200 shadow">
        <div className="text-sm text-gray-600">Total Binary Income</div>
        <div className="text-2xl font-semibold mt-1">${Number(totalBinaryIncome).toFixed(2)}</div>
      </div>
      ...
      <div className="relative overflow-x-auto bg-white rounded-lg shadow border min-h-[400px]">
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {binaryRewards.length === 0 && !isFetching ? (
              <tr><td colSpan={11} className="p-12 text-center text-gray-500">No records found</td></tr>
            ) : (
...
export default BinaryIncomeRewards;